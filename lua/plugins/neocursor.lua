...
return {
  ...
  -- commit = "1699a90e", -- Opcional: Lazy.nvim descargará siempre lo último de main.
  ...
  opts = {
    ...
    usage_host = "antigravity-tui", -- "antigravity" | "antigravity-tui" | "cursor"
  ...
  config = function(_, opts)
    ...
    local uh = opts.usage_host or "cursor"
    vim.g.ai_host = vim.tbl_contains({ "antigravity", "antigravity-rpc" }, uh) and "antigravity"
      or (uh == "antigravity-tui" and "antigravity" or "cursor")
    vim.g.ai_host_tui = uh == "antigravity-tui"
    local usage_scripts = {
      cursor = vim.fn.stdpath("config") .. "/cursor_usage.py",
      antigravity = vim.fn.stdpath("config") .. "/antigravity_usage.py",
    }
    ...
    local function ai_usage()
      if vim.g.ai_host_tui then
        anty_usage_float()
        return
      end
      if vim.g.ai_host == "antigravity" then
        run_usage(usage_scripts.antigravity)
        return
      end
